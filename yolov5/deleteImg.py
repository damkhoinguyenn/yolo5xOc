...
# Find image files that do not have a matching label file
images_to_delete = [image for image in image_base_names if image not in label_base_names]

# Now, construct the full paths to the files to be deleted
full_paths_to_delete = [os.path.join(image_directory, image + '.jpg') for image in images_to_delete]

# Initialize progress counters
total_files = len(full_paths_to_delete)
deleted_count = 0

print(f"Starting deletion of {total_files} files...")
...
for file_path in full_paths_to_delete:
    if os.path.exists(file_path):
        ...
        print(f"File {file_path} does not exist and cannot be deleted.")
